...
class multiLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # h_0 = Variable(torch.zeros(self.num_layers, x.size(0), self.hidden_unit_size))  # hidden state initialized with zeros
        # c_0 = Variable(torch.zeros(self.num_layers, x.size(0), self.hidden_unit_size))  # internal state initialized with zeros
    ...
    def TrainViterbiNet(self, channel_train, symbol_train, const_size):
        ...
        # print
        print('-training-')
        print(f'Stacked LSTM Loss: {running_loss.mean()}')
